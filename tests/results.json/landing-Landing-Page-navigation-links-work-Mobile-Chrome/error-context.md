...
```
Error: locator.click: Error: strict mode violation: getByRole('link', { name: /Pricing/i }) resolved to 2 elements:
    1) <a href="/pricing">…</a> aka getByRole('link', { name: 'View Pricing' })
    2) <a href="/pricing" class="text-sm text-muted-foreground hover:text-primary transition-colors">Pricing</a> aka getByRole('link', { name: 'Pricing', exact: true })
